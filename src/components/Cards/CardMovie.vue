	...
	<div class="card-fluid card-config">
		<div class="poster-container">
			...
			<div class="poster-wd">
				<img
					:src="
						$store.state.Movies.currentMovie.poster
							? $store.state.Movies.currentMovie.poster
							: `${APP_URL}img/empty-img.png`
					"
					alt="card-filme"
			...
			<div
				...
				v-if="
					$store.state.Movies.currentMovie.imdb.rating &&
					$store.state.Movies.currentMovie.imdb.votes
				"
			>
				<div class="exibition pt-1">
					<span
						>Avaliação ({{ $store.state.Movies.currentMovie.imdb.votes }})</span
					>
					<StarRating class="responsive ms-1" v-model="rating" />
				</div>
			</div>
		...
		<div class="info-movie">
			<header class="card-header">
				<h2>{{ $store.state.Movies.currentMovie.title }}</h2>
				<div class="d-flex gap-2">
					<div
						v-for="Movie in $store.state.Movies.currentMovie.genres"
						:key="Movie.length"
					>
				...
				<p v-if="$store.state.Movies.currentMovie.runtime">
					Duração:
					{{
						Math.trunc($store.state.Movies.currentMovie.runtime / 60) +
						'h' +
				...
				<div class="info-generals-movies">
					<div
						...
						v-if="$store.state.Movies.currentMovie.imdb.rating"
					>
						<Icon icon="la:imdb" />
						<p>{{ $store.state.Movies.currentMovie.imdb.rating }}/10</p>
					</div>
					<div
						class="d-flex icon-styles2"
						v-if="
							$store.state.Movies.currentMovie.tomatoes &&
							$store.state.Movies.currentMovie.tomatoes.viewer &&
							$store.state.Movies.currentMovie.tomatoes.critic
						"
					>
						<Icon icon="simple-icons:rottentomatoes" />
						<p>{{ TomatoesRating.toFixed(2) }}%</p>
					</div>
					<div
						class="d-flex"
						v-if="$store.state.Movies.currentMovie.awards.text"
					>
						<p>{{ $store.state.Movies.currentMovie.awards.text }}</p>
					</div>
				</div>
			</header>
			<div
				v-if="
					$store.state.Movies.currentMovie.trailer
						? $store.state.Movies.currentMovie.trailer
						: hiddenBtnTrailer
				"
